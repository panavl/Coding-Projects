...
class Tree():
    ...
    def get_level(self, level):
        ...
        def get_level_nodes(node, goal_level, current_level):
            if node == None:
                return
            if current_level == goal_level:
                nodes.append(node)
            else:
                get_level_nodes(node.lchild, goal_level, current_level + 1)
                get_level_nodes(node.rchild, goal_level, current_level + 1)

        get_level_nodes(self.root, level, 0)
        return nodes
    ...
    def left_side_view(self):
        left_nodes = []
        self.get_left_side_view(self.root.lchild,left_nodes)
        return left_nodes

    def get_left_side_view(self,node,list_nodes):
        if node != None:
            list_nodes.append(node.data)
            self.get_left_side_view(node.lchild,list_nodes)
            self.get_left_side_view(node.rchild,list_nodes)
    ...
    def sum_leaf_nodes(self):  
        def get_sum_leaf_nodes(self,node):
            if node == None:
                return 0
            if node.lchild == None and node.rchild == None:
                return node.data
            return self.get_sum_leaf_nodes(node.lchild) + self.get_sum_leaf_nodes(node.rchild)
        
        return get_sum_leaf_nodes(self.root)
